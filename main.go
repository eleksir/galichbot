	...
	for update := range bot.Updates() {
		if update.Message != nil {
			...
			if update.Message.From.ID == victim {
				n := rand.Int() % len(speeches) //nolint:gosec

				var options = micha.SendMessageOptions{ReplyToMessageID: update.Message.MessageID} //nolint:exhaustivestruct
